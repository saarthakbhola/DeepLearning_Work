...
while True:
    # Grab a single frame of videov
    ret, frame = video_capture.read()

    # Find all the faces and face enqcodings in the frame of video
    face_locations = facial_recognition.face_locations(frame)
    face_encodings = facial_recognition.face_encodings(frame, face_locations)

    # Loop through each face in this frame of video
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        # See if the face is a match for the known face(s)
        match = facial_recognition.compare_faces([Batman_face_encoding], face_encoding)
